...
- Hover and focus now fade over 140ms instead of snapping. background-image is
  not animatable, so a gradient cannot transition into another one directly.
  The gradients are built from registered custom properties and the alpha is
  what animates, which the browser can interpolate.
- README section on the tech stack and why there is no framework, no build step
  and no dependencies.
...
- The popup now uses the logo's colours. Headings and the brand name carry the
  amber-to-red gradient, and the Save button is filled with it. The accent was
  still the blue placeholder from before the logo existed, so nothing on screen
  matched the icon.
- Every button has a visible border or fill, so none of them read as plain text.
- About and Back collapsed into one toggle button in the bottom-right of the
  footer, bordered and aligned against the version on the opposite edge.
...
[1.10.0]: https://github.com/badjuice/TabsExplode/releases/tag/v1.10.0
